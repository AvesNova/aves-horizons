...
class TeamTransformerModel(nn.Module):
    ...
    def __init__(
        ...
        num_actions: int = 6,  # Number of possible actions per ship
...
def create_team_model(config: dict = None) -> TeamTransformerModel:
    """Factory function to create a team transformer model with good defaults."""
    default_config = {
        "token_dim": 10,
        "embed_dim": 64,
        "num_heads": 4,
    ...
    return TeamTransformerModel(**default_config)


def example_usage():
    """Example of how to use the team transformer model."""

    # Create model
    model = create_team_model(
        {
            "max_ships": 4,
            "embed_dim": 32,  # Smaller for example
        }
    )
    ...
    team_controller = TeamController(
        {
            0: [0, 1],  # Team 0 controls ships 0 and 1
            1: [2, 3],  # Team 1 controls ships 2 and 3
        }
    )
    ...
    batch_size = 2
    max_ships = 4
    observation = {"tokens": torch.randn(batch_size, max_ships, 10)}

    # Get ship mask (mark inactive ships)
    device = torch.device("cpu")
    ship_mask = team_controller.get_ship_mask(batch_size, max_ships, device)

    # Forward pass
    with torch.no_grad():
        output = model(observation, ship_mask)
        print("Action logits shape:", output["action_logits"].shape)

        # Get actions
        action_output = model.get_actions(observation, ship_mask, temperature=1.0)
        all_actions = action_output["actions"]

        # Extract actions for each team
        team_0_actions = team_controller.extract_team_actions(all_actions, team_id=0)
        team_1_actions = team_controller.extract_team_actions(all_actions, team_id=1)

        print(
            "Team 0 actions:",
            {ship_id: action.shape for ship_id, action in team_0_actions.items()},
        )
        print(
            "Team 1 actions:",
            {ship_id: action.shape for ship_id, action in team_1_actions.items()},
        )
